...
function test_get_reduced_costs(model::DantzigLP.DantzigModel)
    actual_costs = vcat(getdual(model.pos_betas),
                        getdual(model.neg_betas))
    test_pos_costs, test_neg_costs = DantzigLP.get_reduced_costs(model)
...
function test_optimality(model::DantzigLP.DantzigModel, β, baseline_β)
    @test objective(model, β) ≈ objective(model, baseline_β) atol=ε
end


function objective(model::DantzigLP.DantzigModel, β)
    return norm(β, 1)
end


function objective(model::DantzigLP.GroupDantzigModel, β)
    return sum([norm(βs, Inf) for (group, βs) in DantzigLP.split_by(β, model.groups)])
end
...
function test_feasibility(model::DantzigLP.GroupDantzigModel, problem, β)
    X, y, g, λ = problem
    Xtr = (X') * (y - X * β)
...
for algo in algos
    for scenario in scenarios
        problem, model, β, baseline_β = algo(; scenario...)
